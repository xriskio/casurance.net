...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Office Building Quotes
  app.post("/api/office-quotes", async (req, res) => {
    try {
      const validatedData = insertOfficeQuoteSchema.parse(req.body);
      const referenceNumber = generateReferenceNumber('RFQ');
      const quote = await storage.createOfficeQuote({ ...validatedData, referenceNumber } as any);
      ...
      const emailData = {
        ...
        email: validatedData.email,
        phone: validatedData.phone,
        insuranceType: 'Office Building Insurance'
      };
